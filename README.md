...
# Быстрый старт
predictor = HousingPricePredictor()
price = predictor.quick_predict(area=65, rooms=3, district='СВАО')
print(f"Оценочная стоимость: {price:,.0f} ₽")
```

### Продвинутый пример с автообновлением

```python
from moscow_housing_ml import HousingPricePredictor, CianParser
import schedule
import time
...
def auto_update():
    ...
    print("Обновление данных с ЦИАН...")
    fresh_data = parser.fetch_latest_listings()
    predictor.update_with_fresh_data(fresh_data)
    print(f"Обновлено {len(fresh_data)} новых объявлений")
